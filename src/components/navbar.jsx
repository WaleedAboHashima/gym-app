...
const Navbar = ({bar,setBar,setActiveBar,setActive,active,activeBar}) => {
...
const [activeMenu,setActiveMenu] = useState(false)
  ...
  const handleLogOut = () => {
    ...
    console.log(cookies.get("_auth_role"));
  }
  return (
    <>
      ...
      <div className='flex border border-black rounded mx-2' >
        ...
        ("reg")} >تسجيل</Link>
          ...
          <div>
            <img src={logo} alt="logo" className='w-50 h-11 cursor-pointer' onClick={() => {
              navigate("/")
              setActive("")
              setActiveBar("/")
            }} />
        ...
        <>
            ...
            {cookies.get("_auth_token") ?
              <div className='flex flex-col relative w-fit'>
                ...
                {activeMenu && <div className={`flex-col flex absolute bg-slate-100 ${cookies.get("_auth_role") === "65100109105110" ? "-bottom-24 " :"-bottom-16 "}  left-6  w-32 rounded-lg`}>
                  {cookies.get("_auth_role") === "65100109105110" ?<><span className='text-xl w-full text-center  hover:bg-neutral-700 transition-all hover:text-white cursor-pointer p-2' onClick={() => {
                    setActiveMenu(false)
                    setActiveBar("")
                    navigate("/admin/clubs")
                    checkActive()
                  }} >لوحه التحكم</span>
                                    <span className='text-xl w-full text-center hover:bg-neutral-700 transition-all hover:text-white cursor-pointer p-2' onClick={handleLogOut} >نسجيل الخروج</span>
</> :                  <span className='text-xl w-full text-center hover:bg-neutral-700 transition-all hover:text-white cursor-pointer p-2' onClick={handleLogOut} >نسجيل الخروج</span> }
          ...
          <ul className='flex gap-x-10 flex-col sm:flex-row'>
            ...
            <div ><img src={logo} alt="logo" className='w-50 h-11 cursor-pointer' onClick={() => {navigate("/");checkActive()}}/></div>
          </div>
          <div className='md:hidden flex justify-between p-3 h-18'>
          <img src={logo} alt="logo" className='w-50 h-11 cursor-pointer' onClick={() => {navigate("/");checkActive()}}/>
